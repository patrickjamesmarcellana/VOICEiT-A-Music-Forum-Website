...
async function test() {
    ...
    // populate comments
    const hardcoded_comment1004 = await (new Comment({ post_id: hardcoded_post10, subcomments: [], author: hardcoded_user4._id, body: "yeah, it's such a broad term that it lost its original meaning" } )).save()
    const hardcoded_comment1003 = await (new Comment({ post_id: hardcoded_post10, subcomments: [hardcoded_comment1004], author: hardcoded_user3._id, body: "When every band, even the popular ones got marketed as \"indie\", that word just completely lost its meaning." } )).save()
    const hardcoded_comment1002 = await (new Comment({ post_id: hardcoded_post10, subcomments: [hardcoded_comment1003], author: hardcoded_user2._id, body: "Got to be honest, aren't most mainstream rock bands labeled as \"alternative\" or \"indie\" despite them not actually being alternative or independent? Say, like, Radiohead." } )).save()
    const hardcoded_comment1001 = await (new Comment({ post_id: hardcoded_post10, subcomments: [], author: hardcoded_user1._id, body: "You have to understand that most people aren't really passionate about music and will just follow whatever the ongoing trend or fad is. They're only interested in whatever's popular. But you shouldn't care about that. Screw them. Just do you, and your people and the community you belong to will find you." } )).save()
    const hardcoded_comment904 = await (new Comment({ post_id: hardcoded_post9, subcomments: [], author: hardcoded_user5._id, body: "You'll never find a good country song on the radio. You actually have to actively seek them out. Listen to Colter Wall and Orville Peck on Spotify and you'll see the difference." } )).save()
    const hardcoded_comment903 = await (new Comment({ post_id: hardcoded_post9, subcomments: [], author: hardcoded_user3._id, body: "The rednecks, the cowboy boots, rednecks, line dancing, rednecks, lyrics no man on Earth would be able to relate to, rednecks, and the extremely simple and repetitive song structure. Oh, did I forget to mention the rednecks?" + "\n" + "Country music is the same old, soulless song filled with boring, bland, and dull cliches produced over and over again by corporations with clearly no regard for quality. There are exceptions, of course, but then again, they are exceptions, not the rule." } )).save()
    const hardcoded_comment902 = await (new Comment({ post_id: hardcoded_post9, subcomments: [], author: hardcoded_user2._id, body: "I highly agree on this. It's just the same song copy and pasted without any innovation. It's become like hip-hop imo" } )).save()
    const hardcoded_comment901 = await (new Comment({ post_id: hardcoded_post9, subcomments: [hardcoded_comment902], author: hardcoded_user1._id, body: "I don't know about you but I hear this one guitar tone in literally every country song known to mankind. If there's anything that would put someone off, it's definitely one of the reasons." } )).save()
    const hardcoded_comment803 = await (new Comment({ post_id: hardcoded_post8, subcomments: [], author: hardcoded_user4._id, body: "The only answer you need is Bach. And more Bach." } )).save()
    const hardcoded_comment802 = await (new Comment({ post_id: hardcoded_post8, subcomments: [], author: hardcoded_user3._id, body: `Bach: Goldberg Variations \n Mozart: Symphonies 25, 40, and 41\n Beethoven: Symphonies 3, 5, 7, and 9\n Chopin: …everything.\n Tchaikovsky: Symphony No. 4\n Debussy: Clair De Lune\n ` } )).save()
    const hardcoded_comment801 = await (new Comment({ post_id: hardcoded_post8, subcomments: [], author: hardcoded_user2._id, body: "Most definitely start with Bach and Mozart so you can get a good feel of the genre and know what to expect from that point. People might say try Tchaikovsky but I feel like his music is just flashy and is unsuited for a beginner. Maybe when you've grown accustomed to the entire thing." } )).save()
    const hardcoded_comment704 = await (new Comment({ post_id: hardcoded_post7, subcomments: [], author: hardcoded_user4._id, body: "Yes, fantastic album. Absolute classic." } )).save()
    const hardcoded_comment703 = await (new Comment({ post_id: hardcoded_post7, subcomments: [hardcoded_comment704], author: hardcoded_user3._id, body: "Idle Moments by Grant Green, no doubt about it." } )).save()
    const hardcoded_comment702 = await (new Comment({ post_id: hardcoded_post7, subcomments: [], author: hardcoded_user5._id, body: "good taste, was just about to comment the same thing" } )).save()
    const hardcoded_comment701 = await (new Comment({ post_id: hardcoded_post7, subcomments: [hardcoded_comment702], author: hardcoded_user2._id, body: "Charles Mingus - Mingus Ah Um" } )).save()
    const hardcoded_comment604 = await (new Comment({ post_id: hardcoded_post6, subcomments: [], author: hardcoded_user5._id, body: "I remember him performing during Royal Rumble 2022. What an absolute waste of time and money. It's just like Maroon 5 and Travis Scott during the Super Bowl LIII halftime show. Unbelievable." } )).save()
    const hardcoded_comment603 = await (new Comment({ post_id: hardcoded_post6, subcomments: [], author: hardcoded_user3._id, body: "Have this downvote." } )).save()
    const hardcoded_comment602 = await (new Comment({ post_id: hardcoded_post6, subcomments: [hardcoded_comment603], author: hardcoded_user2._id, body: "no, I agree with him" } )).save()
    const hardcoded_comment601 = await (new Comment({ post_id: hardcoded_post6, subcomments: [hardcoded_comment602], author: hardcoded_user1._id, body: "That's just like, your opinion, man. A very shitty one, too." } )).save()
    const hardcoded_comment504 = await (new Comment({ post_id: hardcoded_post5, subcomments: [], author: hardcoded_user5._id, body: "They already signed with Warner Records, so I think they'll do fine" } )).save()
    const hardcoded_comment503 = await (new Comment({ post_id: hardcoded_post5, subcomments: [hardcoded_comment504], author: hardcoded_user4._id, body: "Hope they could make something out of this and not just become another artist or group that became viral on TikTok. I wish them all the best." } )).save()
    const hardcoded_comment502 = await (new Comment({ post_id: hardcoded_post5, subcomments: [], author: hardcoded_user1._id, body: "Yes. As much as I love Cupid, I just think Higher is their best song yet. I honestly have trouble getting it out of my head." } )).save()
    const hardcoded_comment501 = await (new Comment({ post_id: hardcoded_post5, subcomments: [hardcoded_comment502], author: hardcoded_user2._id, body: "So happy for them. Hope they can keep this up and their other songs get popular as well." } )).save()
    const hardcoded_comment403 = await (new Comment({ post_id: hardcoded_post4, subcomments: [], author: hardcoded_user4._id, body: "I like it, but it's definitely too long. CTRL > SOS. 7-8/10" } )).save()
    const hardcoded_comment402 = await (new Comment({ post_id: hardcoded_post4, subcomments: [], author: hardcoded_user2._id, body: "Loved it. Even the ones I skipped on re-listens, I like them." } )).save()
    const hardcoded_comment401 = await (new Comment({ post_id: hardcoded_post4, subcomments: [], author: hardcoded_user3._id, body: "I honestly loved it. It's the perfect post-breakup album, so it really spoke to me. I love her so much." } )).save()
    const hardcoded_comment303 = await (new Comment({ post_id: hardcoded_post3, subcomments: [], author: hardcoded_user5._id, body: "Nah, pop-rock is Blink hardcoded_comment182, early One Republic, Fall Out Boy, and Coldplay, not that garbage" } )).save()
    const hardcoded_comment302 = await (new Comment({ post_id: hardcoded_post3, subcomments: [hardcoded_comment303], author: hardcoded_user4._id, body: "They're usually considered pop-rock, but they're just stylish pop imo" } )).save()
    const hardcoded_comment301 = await (new Comment({ post_id: hardcoded_post3, subcomments: [hardcoded_comment302], author: hardcoded_user3._id, body: "Wait, Imagine Dragons is a rock band?" } )).save()
    const hardcoded_comment204 = await (new Comment({ post_id: hardcoded_post2, subcomments: [], author: hardcoded_user4._id, body: "frfr" } )).save()
    const hardcoded_comment203 = await (new Comment({ post_id: hardcoded_post2, subcomments: [hardcoded_comment204], author: hardcoded_user3._id, body: "Recovery gang" } )).save()
    const hardcoded_comment202 = await (new Comment({ post_id: hardcoded_post2, subcomments: [], author: hardcoded_user1._id, body: "L take" } )).save()
    const hardcoded_comment201 = await (new Comment({ post_id: hardcoded_post2, subcomments: [hardcoded_comment202], author: hardcoded_user5._id, body: "Relapse, hands down. Better production, flows, and lyrics. Recovery has its hooks and choruses literally catered to 14-year-old girls. Em hasn't topped Relapse yet. Screw Recovery. " } )).save()
    const hardcoded_comment106 = await (new Comment({ post_id: hardcoded_post1, subcomments: [], author: hardcoded_user5._id, body: "I don't think it's too EDM-y like that one" } )).save()
    const hardcoded_comment105 = await (new Comment({ post_id: hardcoded_post1, subcomments: [hardcoded_comment106], author: hardcoded_user4._id, body: "Roses by the Chainsmokers maybe?" } )).save()
    const hardcoded_comment104 = await (new Comment({ post_id: hardcoded_post1, subcomments: [], author: hardcoded_user5._id, body: "Sorry, it's not it" } )).save()
    const hardcoded_comment103 = await (new Comment({ post_id: hardcoded_post1, subcomments: [hardcoded_comment104], author: hardcoded_user3._id, body: "This has to be Zedd - Clarity. Check the part around 1:09" } )).save()
    const hardcoded_comment102 = await (new Comment({ post_id: hardcoded_post1, subcomments: [], author: hardcoded_user2._id, body: "lmao I seriously can't tell if you're being sarcastic or not, but I just woke up my kids from laughing so hard" } )).save()
    const hardcoded_comment101 = await (new Comment({ post_id: hardcoded_post1, subcomments: [hardcoded_comment102], author: hardcoded_user1._id, body: "You sure it's not Bastille - Pompeii?" } )).save()
